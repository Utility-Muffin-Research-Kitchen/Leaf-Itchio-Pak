...
func TestSaveStatePaths_Format1_GB(t *testing.T) {
	...
	base := "/leaf/States/GB-gambatte/Doomslinger Dungeon"
...
func TestSaveStatePaths_ZipWithInnerFilename_Format0(t *testing.T) {
	romPath := "/leaf/Roms/GB/Pokemon - Red Version (USA, Europe).zip"
	inner := "Pokemon - Red Version (USA, Europe).gb"
	paths := roms.SaveStatePaths(romPath, 0, inner, "GB", "gambatte")
	if len(paths) != 10 {
		t.Fatalf("got %d paths", len(paths))
	}
	base := "/leaf/States/GB-gambatte/Pokemon - Red Version (USA, Europe).gb"
	if paths[0] != base+".st0" {
...
func TestSaveStatePaths_UnrecognisedDir_ReturnsNil(t *testing.T) {
	paths := roms.SaveStatePaths("/leaf/Roms/Unknown/foo.rom", 0, "", "", "")
	if paths != nil {
...
func TestRomCoreInfo_NES(t *testing.T) {
	tag, core := roms.RomCoreInfo("/leaf/Roms/NES/game.nes")
...
func TestRomCoreInfo_Genesis(t *testing.T) {
	tag, core := roms.RomCoreInfo("/leaf/Roms/GENESIS/game.md")
