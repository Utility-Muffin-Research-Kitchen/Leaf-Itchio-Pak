...
// coreTag / coreName identify the Leaf core state directory.
...
func SaveStatePaths(romDestPath string, stateFormat int, innerFilename, coreTag, coreName string) []string {
	...
	statesDir := filepath.Join(StatesRoot(), coreTag+"-"+coreName)
...
func RomCoreInfo(romDestPath string) (coreTag, coreName string) {
	dir := withTrailingSlash(filepath.Dir(romDestPath))
	switch dir {
	case SystemDir("GB"):
		return "GB", "gambatte"
	case SystemDir("GBC"):
