...
type MaskedKeyboardScreen struct {
	ctx   *Context
	ui    *Composer
	model *appui.MaskedKeyboardModel
}

func NewMaskedKeyboardScreen(ctx *Context, model *appui.MaskedKeyboardModel) (*MaskedKeyboardScreen, error) {
	ui, err := NewComposer(ctx)
	if err != nil {
		return nil, err
	}
	return &MaskedKeyboardScreen{ctx: ctx, ui: ui, model: model}, nil
}

func (screen *MaskedKeyboardScreen) HandleInput(event InputEvent) appui.MaskedKeyboardIntent {
	if event.Wake {
		return appui.MaskedKeyboardIntentNone
	}
	return screen.model.Handle(appui.InputEvent{Button: appButton(event.Button), Pressed: event.Pressed, Repeated: event.Repeated})
}

func (screen *MaskedKeyboardScreen) Draw() error {
	footer := []FooterHint{
		{Button: ButtonB, Label: "Delete"}, {Button: ButtonX, Label: "Case"},
		{Button: ButtonY, Label: "Cancel"}, {Button: ButtonStart, Label: "Save", IsConfirm: true},
	}
	frame, err := screen.ui.BeginScreen(ScreenSpec{Title: screen.model.Title, Footer: footer})
	if err != nil {
		return err
	}
	body := frame.Layout.Content
	field := body.CarveTop(screen.ctx.Scale(62)).Content()
	if err := screen.ui.DrawTextField(field, "API Key", screen.model.Masked(), true); err != nil {
		return err
	}
	body.CarveTop(screen.ui.BasePadding)
	if err := screen.ui.DrawKeyboard(body.Content(), screen.model.Keys(), screen.model.Selected); err != nil {
		return err
	}
	return frame.Finish()
}
