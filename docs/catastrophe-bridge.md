...
Search uses the standard Catastrophe keyboard through the bridge's blocking
`Context.Keyboard` call on the owner thread and may supply the current staged
query. QR sign-in needs no keyboard. App-local keyboard drawing is fixture-only.
